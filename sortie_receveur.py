...
########### a besoin pour fonctionner du fichier nex_transfert obtenua l'aide de code.py
###############
################## Importation ##################
...
ale_1 = open("new_transfert_receveur.txt","r")
...
try:
    ...
    for i in lignes_ale_2:
        ...
        transfert_ale_2.append(receveur_a[0])
        score = info[2].split()
    ...
    for j in range(len(transfert_ale_1)):
        if i!=j and j>i:
            if transfert_ale_1[i] == transfert_ale_1[j]:
                score_1[i] = float(score_1[i])+float(score_1[j])
    ...
    for j in range(len(transfert_ale_2)):
        if i!=j and j>i:
            if transfert_ale_2[i] == transfert_ale_2[j]:
                score_2[i] = float(score_2[i])+float(score_2[j])
